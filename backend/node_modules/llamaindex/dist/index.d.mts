...
declare function jsonToNode(json: any): TextNode<Metadata>;
/**
...
declare class OpenAI implements LLM {
    ...
    get metadata(): {
        model: "gpt-3.5-turbo" | "gpt-3.5-turbo-16k" | "gpt-4" | "gpt-4-32k" | "gpt-3.5-turbo-1106" | "gpt-4-1106-preview" | "gpt-4-vision-preview";
        temperature: number;
        topP: number;
 ...
 */
declare function getNodesFromDocument(document: Document, textSplitter: SentenceSplitter, includeMetadata?: boolean, includePrevNextRel?: boolean): TextNode<Metadata>[];
/**
...
declare class SimpleNodeParser implements NodeParser {
     ...
     * @param documents
     */
    getNodesFromDocuments(documents: Document[]): TextNode<Metadata>[];
}
...
declare class RetrieverQueryEngine implements BaseQueryEngine {
    ...
    preFilters?: unknown;
    constructor(retriever: BaseRetriever, responseSynthesizer?: ResponseSynthesizer, preFilters?: unknown, nodePostprocessors?: BaseNodePostprocessor[]);
...
interface ChatEngine {
     ...
     */
    chat<T extends boolean | undefined = undefined, R = T extends true ? AsyncGenerator<string, void, unknown> : Response>(message: string, chatHistory?: ChatMessage[], streaming?: T): Promise<R>;
    /**
     * Resets the chat history so that it's empty.
     */
    reset(): void;
...
declare class SimpleChatEngine implements ChatEngine {
    ...
    chat<T extends boolean | undefined = undefined, R = T extends true ? AsyncGenerator<string, void, unknown> : Response>(message: string, chatHistory?: ChatMessage[], streaming?: T): Promise<R>;
    protected streamChat(message: string, chatHistory?: ChatMessage[]): AsyncGenerator<string, void, unknown>;
    reset(): void;
}
/**
...
declare class CondenseQuestionChatEngine implements ChatEngine {
    ...
    chat<T extends boolean | undefined = undefined, R = T extends true ? AsyncGenerator<string, void, unknown> : Response>(message: string, chatHistory?: ChatMessage[] | undefined, streaming?: T): Promise<R>;
    reset(): void;
}
interface Context {
    message: ChatMessage;
...
declare class ContextChatEngine implements ChatEngine {
    ...
    chat<T extends boolean | undefined = undefined, R = T extends true ? AsyncGenerator<string, void, unknown> : Response>(message: string, chatHistory?: ChatMessage[] | undefined, streaming?: T): Promise<R>;
    protected streamChat(message: string, chatHistory?: ChatMessage[] | undefined): AsyncGenerator<string, void, unknown>;
    reset(): void;
}
...
declare class VectorStoreIndex extends BaseIndex<IndexDict> {
     ...
     * @returns
     */
    static getNodeEmbeddingResults(nodes: BaseNode[], serviceContext: ServiceContext, logProgress?: boolean): Promise<BaseNode<Metadata>[]>;
    /**
...
export { ALL_AVAILABLE_ANTHROPIC_MODELS, ALL_AVAILABLE_LLAMADEUCE_MODELS, ALL_AVAILABLE_OPENAI_MODELS, Anthropic, AnthropicStreamToken, BaseDocumentStore, BaseEmbedding, BaseInMemoryKVStore, BaseIndex, BaseIndexInit, BaseIndexStore, BaseKVStore, BaseNode, BaseNodePostprocessor, BaseOutputParser, BaseQueryEngine, BaseQuestionGenerator, BaseReader, BaseRetriever, BaseTool, CallbackManager, ChatEngine, ChatHistory, ChatMessage, ChatResponse, ChoiceSelectPrompt, CompactAndRefine, CompleteFileSystem, CompletionResponse, CondenseQuestionChatEngine, CondenseQuestionPrompt, Context, ContextChatEngine, ContextGenerator, ContextSystemPrompt, DEFAULT_CHUNK_OVERLAP, DEFAULT_CHUNK_OVERLAP_RATIO, DEFAULT_CHUNK_SIZE, DEFAULT_COLLECTION, DEFAULT_CONTEXT_WINDOW, DEFAULT_DOC_STORE_PERSIST_FILENAME, DEFAULT_EMBEDDING_DIM, DEFAULT_FS, DEFAULT_GRAPH_STORE_PERSIST_FILENAME, DEFAULT_INDEX_STORE_PERSIST_FILENAME, DEFAULT_NAMESPACE, DEFAULT_NUM_OUTPUTS, DEFAULT_PADDING, DEFAULT_PERSIST_DIR, DEFAULT_SIMILARITY_TOP_K, DEFAULT_VECTOR_STORE_PERSIST_FILENAME, DefaultContextGenerator, DefaultStreamToken, DeuceChatStrategy, Document, Event, EventTag, EventType, ExactMatchFilter, FILE_EXT_TO_READER, GPT35_MODELS, GPT4_MODELS, GenericFileSystem, HTMLReader, HistoryChatEngine, InMemoryFileSystem, IndexDict, IndexList, IndexNode, IndexStruct, IndexStructType, KeywordExtractPrompt, KeywordTable, KeywordTableIndex, KeywordTableLLMRetriever, KeywordTableRAKERetriever, KeywordTableRetrieverMode, KeywordTableSimpleRetriever, LLM, LLMMetadata, LLMQuestionGenerator, ListIndex, ListIndexLLMRetriever, ListIndexRetriever, ListRetrieverMode, LlamaDeuce, MarkdownReader, MessageContent, MessageContentDetail, MessageType, Metadata, MetadataFilters, MetadataInfo, MetadataMode, NodeParser, NodeRelationship, NodeWithScore, NotionReader, ObjectType, OpenAI, OpenAIEmbedding, OpenAIStreamToken, PDFReader, PapaCSVReader, Portkey, PromptHelper, QueryEngineTool, QueryKeywordExtractPrompt, RefDocInfo, Refine, RefinePrompt, RelatedNodeInfo, RelatedNodeType, Response, ResponseSynthesizer, RetrievalCallbackResponse, RetrieverQueryEngine, SentenceSplitter, ServiceContext, ServiceContextOptions, SimilarityPostprocessor, SimilarityType, SimpleChatEngine, SimpleChatHistory, SimpleDirectoryReader, SimpleDirectoryReaderLoadDataProps, SimpleDocumentStore, SimpleIndexStore, SimpleKVStore, SimpleNodeParser, SimplePrompt, SimpleResponseBuilder, SimpleVectorStore, StorageContext, StreamCallbackResponse, StructuredOutput, SubQuestion, SubQuestionOutputParser, SubQuestionPrompt, SubQuestionQueryEngine, SummaryChatHistory, SummaryIndex, SummaryIndexLLMRetriever, SummaryIndexRetriever, SummaryPrompt, SummaryRetrieverMode, TextFileReader, TextNode, TextQaPrompt, Tokenizers, ToolMetadata, TreeSummarize, TreeSummarizePrompt, VectorIndexRetriever, VectorStore, VectorStoreIndex, VectorStoreInfo, VectorStoreQuery, VectorStoreQueryMode, VectorStoreQueryResult, VectorStoreQuerySpec, WalkableFileSystem, buildToolsText, cjkSentenceTokenizer, defaultChoiceSelectPrompt, defaultCondenseQuestionPrompt, defaultContextSystemPrompt, defaultKeywordExtractPrompt, defaultQueryKeywordExtractPrompt, defaultRefinePrompt, defaultSubQuestionPrompt, defaultSummaryPrompt, defaultTextQaPrompt, defaultTreeSummarizePrompt, englishSentenceTokenizer, exists, getBiggestPrompt, getEmptyPromptTxt, getNodeFS, getNodesFromDocument, getResponseBuilder, getTextSplitsFromDocument, getTopKEmbeddings, getTopKEmbeddingsLearner, getTopKMMREmbeddings, globalsHelper, jsonToIndexStruct, jsonToNode, messagesToHistoryStr, parseJsonMarkdown, serviceContextFromDefaults, serviceContextFromServiceContext, similarity, storageContextFromDefaults, unixLineSeparator, unixParagraphSeparator, walk, windowsLineSeparator, windowsParagraphSeparator };
