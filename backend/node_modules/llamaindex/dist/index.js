...
__export(src_exports, {
  ALL_AVAILABLE_ANTHROPIC_MODELS: () => ALL_AVAILABLE_ANTHROPIC_MODELS,
  ...
  HistoryChatEngine: () => HistoryChatEngine,
  ...
  SimpleChatHistory: () => SimpleChatHistory,
  ...
  getEmptyPromptTxt: () => getEmptyPromptTxt,
  getNodeFS: () => getNodeFS,
...
var OpenAISession = class {
  constructor(options = {}) {
    ...
    if (options.azure) {
      ...
      this.openai = new import_openai.default(__spreadValues({}, options));
    }
  }
};
...
var GlobalsHelper = class {
  ...
  initDefaultTokenizer() {
    const encoding = (0, import_js_tiktoken.encodingForModel)("text-embedding-ada-002");
...
var import_portkey_ai = require("portkey-ai");
...
var PortkeySession = class {
  constructor(options = {}) {
    ...
    this.portkey = new import_portkey_ai.Portkey({});
    this.portkey.llms = [{}];
    if (!options.apiKey) {
      throw new Error("Set Portkey ApiKey in PORTKEY_API_KEY env variable");
    }
    this.portkey = new import_portkey_ai.Portkey(options);
  }
};
...
function getPortkeySession(options = {}) {
  ...
  let session = (_a = defaultPortkeySession.find((session2) => {
    return import_lodash3.default.isEqual(session2.options, options);
  })) == null ? void 0 : _a.session;
  if (!session) {
    ...
    return __async(this, null, function* () {
      ...
      if (this.contextGenerator) {
        if (Array.isArray(message)) {
          message = message.filter((c) => c.type === "text").map((c) => c.text).join("\n\n");
        }
        context = yield this.contextGenerator.generate(message);
      }
...
var IndexStruct = class {
  constructor(indexId = (0, import_uuid4.v4)(), summary = void 0) {
...
var InMemoryFileSystem = class {
  ...
  writeFile(path5, content, options) {
    return __async(this, null, function* () {
      this.files[path5] = import_lodash4.default.cloneDeep(content);
    });
  }
  readFile(path5, options) {
    return __async(this, null, function* () {
      ...
      return import_lodash4.default.cloneDeep(this.files[path5]);
    });
  }
  access(path5) {
    return __async(this, null, function* () {
  ...
  mkdir(path5, options) {
    return __async(this, null, function* () {
      this.files[path5] = import_lodash4.default.get(this.files, path5, null);
    });
  }
};
function getNodeFS() {
...
var SimpleDocumentStore = class _SimpleDocumentStore extends KVDocumentStore {
  ...
  persist() {
    return __async(this, arguments, function* (persistPath = path2.join(
      ...
      fs2 = fs2 || DEFAULT_FS;
      if (import_lodash5.default.isObject(this.kvStore) && this.kvStore instanceof BaseInMemoryKVStore) {
  ...
  toDict() {
    if (import_lodash5.default.isObject(this.kvStore) && this.kvStore instanceof SimpleKVStore) {
...
var KVIndexStore = class extends BaseIndexStore {
  ...
  getIndexStruct(structId) {
    return __async(this, null, function* () {
      if (import_lodash6.default.isNil(structId)) {
        ...
        if (import_lodash6.default.isNil(json)) {
          return;
        }
  ...
  getIndexStructs() {
    return __async(this, null, function* () {
      ...
      return import_lodash6.default.values(jsons).map((json) => jsonToIndexStruct(json));
    });
  }
};
...
var SimpleVectorStore = class _SimpleVectorStore {
  ...
  query(query) {
    return __async(this, null, function* () {
      if (!import_lodash7.default.isNil(query.filters)) {
        throw new Error(
...
var import_rake_modified = __toESM(require("rake-modified"));
...
function rakeExtractKeywords(textChunk, maxKeywords) {
  const keywords = Object.keys((0, import_rake_modified.default)(textChunk));
...
var defaultParseChoiceSelectAnswerFn = (answer, numChoices, raiseErr = false) => {
  const lineTokens = answer.split("\n").map((line) => {
    ...
    return lineTokens2;
  }).filter((lineTokens2) => !import_lodash8.default.isNil(lineTokens2));
    ...
    return __async(this, null, function* () {
      ...
      for (let idx = 0; idx < nodeIds.length; idx += this.choiceBatchSize) {
        ...
        const nodeWithScores = choiceNodes.map((node, i) => ({
          node,
          score: import_lodash9.default.get(parseResult, `${i + 1}`, 1)
        }));
  ...
  getRefDocInfo() {
    return __async(this, null, function* () {
      ...
      for (const node of nodes) {
        ...
        if (import_lodash10.default.isNil(refNode)) {
          continue;
        }
        const refDocInfo = yield this.docStore.getRefDocInfo(refNode.nodeId);
        if (import_lodash10.default.isNil(refDocInfo)) {
          continue;
        }
...
var import_notion_md_crawler = require("notion-md-crawler");
var NotionReader = class {
  ...
  constructor({ client, serializers }) {
    this.crawl = (0, import_notion_md_crawler.crawler)({ client, serializers });
  }
  /**
  ...
  toDocuments(pages) {
    return Object.values(pages).map((page) => {
      const text = (0, import_notion_md_crawler.pageToString)(page);
...
var DocxReader = class {
  ...
  loadData(_0) {
    return __async(this, arguments, function* (file, fs2 = DEFAULT_FS) {
      const dataBuffer = yield fs2.readFile(file);
      const { value } = yield import_mammoth.default.extractRawText({ buffer: dataBuffer });
    ...
    return __async(this, arguments, function* ({
      ...
      try {
        for (var iter = __forAwait(walk(fs2, directoryPath)), more, temp, error; more = !(temp = yield iter.next()).done; more = false) {
          ...
          try {
            const fileExt = import_lodash11.default.last(filePath.split(".")) || "";
            ...
            } else if (!import_lodash11.default.isNil(defaultReader)) {
